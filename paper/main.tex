...
\noindent Two things are asserted here. First, a metacognitive output is an \emph{interpretation} formed by \emph{integrating} internal material (thoughts, feelings, sensations) with external material (others' intentions, norms, cues). Second, that interpretation feeds \emph{regulation}. I therefore split the construct into an output (the interpretation) and a controller (the regulation). The output is a \emph{probability} rather than a fixed answer because of how the chapter says interpretations are \emph{formed}: not by retrieving a single verdict, but by entertaining several candidate readings of the same situation, weighing each against the evidence of the moment, and adopting---provisionally---the most believable one. That hypothesis-generating loop is stated as the chapter's own operating sequence (see also p.~3, ``halt premature closure $\rightarrow$ generate alternatives $\rightarrow$ check key evidence,'' and Box~2's ``Name two other explanations,'' p.~12):
% [2026-07-07 반영 C1: probability 설정의 동기를 "가설 생성-증거 갱신-최선 가설 선택"(연구노트의 본래 의도)으로 교체.
%  fact check 결과: 가설 능력 기술은 원문 p.3(halt premature closure→generate alternatives→check key evidence),
%  p.6 각주 2(micro-cycle), p.12 Box 2("Name two other explanations")에서 확인됨. 인용 box도 p.5(JTC)에서 p.6 각주 2로 교체 —
%  부수 효과로, p.5 인용문이 over-externalization 문단 소속이라는 감사 F2의 맥락 문제도 함께 해소.]

\begin{fromtext}{p.~6, footnote 2}
\dots a short, repeated sequence---check confidence; generate two or three alternatives; contrast key evidence (for/against); reweight contextual cues; recalibrate confidence\dots
\end{fromtext}

\noindent Formally, the person entertains candidate hypotheses $h_1,\dots,h_n$ with momentary confidences $p_1,\dots,p_n$ ($\sum_i p_i=1$), and the \emph{interpretation} acted upon is the current best one, $X=\arg\max_i p_i$---held with the awareness that it is a probabilistic choice, not a certainty. Treating confidence as a subjective probability that one's reading is correct is the standard move in computational accounts of metacognition \citep{FlemingDaw2017}, and Proust's own theoretical sketch of procedural metacognition, the \emph{adaptive accumulator}, is a device that accumulates exactly such graded evidence \citep{Proust2012}. Premature certainty---committing before the alternatives have been weighed---then re-appears as the pathology the chapter calls the ``feeling = fact'' shortcut and premature closure (JTC; p.~5). For the single ambiguous social event studied here there are two readings---$h_1$ (benign) and $h_2$ (threat / self-referential)---so one number carries the whole distribution: the posterior probability of the threat reading, written on the log-odds scale:
\begin{equation}
...
\label{eq:control}
\end{equation}
Reading metacognition as a two-level loop---an object level that does the interpreting and a meta level that monitors and steers it---is the founding architecture of the field \citep{NelsonNarens1990}, and it is not a metaphor imported from engineering: Nelson and Narens ground it in the good-regulator argument that a controller must carry a model of what it controls \citep{ConantAshby1970}, Wells' clinical model of the metacognitive control system is explicitly cybernetic \citep{Wells2019}, and the chapter itself later speaks of ``the control-theoretic set point and gain'' (p.~10). Equation \eqref{eq:control} is that architecture written in continuous time. The chapter also tells us what the controller \emph{listens to}: Proust's procedural signals.
...
\noindent This sentence gives the model its central moving part. Evidence arrives on two channels, and the controller \emph{weights} them. Let $\alpha(t)\in[0,1]$ be the weight on the internal channel and $1-\alpha$ the weight on the external channel---equivalently, a convex allocation $(\alpha_{\text{self}},\alpha_{\text{context}})=(\alpha,\,1-\alpha)$ on the simplex, the definition adopted in earlier drafts of this note. The simplex expresses \emph{relative allocation}: how processing is apportioned between self and context at each moment. What it deliberately does not carry is the \emph{overall level} of engagement---the chapter's Figure~2 lets both awarenesses be high (or low) at once, a distinction the allocation alone cannot make. The relation between the two pictures is revisited in \S\ref{sec:disc}. The two channels are:
\begin{align}
...
% [2026-07-07 반영 C4: a_S, a_C, r, ξ(t)의 원문 근거와, 이들이 식 (2)의 자극 S(t)와 어떻게 연결되는지(두 stream 분해) 명시.]
\paragraph{Where these ingredients come from, and how they realize $S(t)$.} The abstract stimulus of Eq.~\eqref{eq:control} enters the person on two streams, $S(t)=\big(S_{\text{int}}(t),\,S_{\text{ext}}(t)\big)$: what the moment stirs up \emph{inside} (affect, associations) and what it offers \emph{outside} (faces, words, behavior). The two channels above are precisely these streams converted into pushes on $L$, and each constant is anchored in the chapter:
\begin{itemize}[leftmargin=1.5em]
\item \textbf{$a_S$ (baseline threat-leaning affect).} The standing internal push, before any looping: ``risk/meaning attributions escalate internally, potentially intensifying anxiety and delusional interpretations'' (p.~5).
\item \textbf{$r\,\sig(L)$ (ruminative gain).} The state-dependent part of the internal stream---the loop of \S\ref{sec:overint} (p.~4--5).
\item \textbf{$a_C$ (disconfirming cue drift).} In this scenario, honestly consulted context speaks \emph{against} the threat reading---this is the chapter's own worked example, where listing context cues moves confidence from 8/10 to 4/10 (Box~2, p.~12). It is a scenario choice, not a law: the chapter equally describes states in which external material is mined \emph{for} threat (hyper-suspiciousness, p.~5; cf.\ ``threat monitoring'' in \citealp{Wells2019}); \S\ref{sec:disc} returns to this.
\item \textbf{$\xi(t)$ (Ornstein--Uhlenbeck noise).} ``The inherent ambiguity of face-to-face situations'' together with the within- and across-day fluctuation the chapter attributes to medication, sleep, and stress (p.~5): evidence that is informative on average but noisy moment to moment, hence time-correlated noise rather than white noise.
\end{itemize}
...
\noindent Because the two axes are \emph{co-regulated} rather than sequenced, belief and dial evolve \emph{together}. In the notation of the research note underlying this manuscript, the abstract system is a coupled, non-autonomous pair
\begin{equation}
...
\label{eq:abstract}
\end{equation}
the first equation the evolution of the interpretation, the second the adaptation of the cognitive referencing system, with $u(t)$ the clinical intervention (pivot triggers, micro-cycles) that forces the system back toward the adaptive corridor. The minimal realization adopted in this note is
...
\label{eq:sysA}
\end{empheq}
% [2026-07-07 반영 C5: (i) 연구노트의 추상 시스템(M, G)을 먼저 제시하고 boxed 식이 그 최소 실현임을 명시,
%  (ii) 이 식이 식 (2)의 M(L,S)를 어떻게 구체화하는지 서술, (iii) alpha가 L의 변화에 따라 조정된다는 결합이
%  u(t)를 경유해 실현됨을 명시(연구노트의 G(α,L,L̇;u) 의존성 = eq:pivot의 b, dL/dt 읽기).]
Equation \eqref{eq:sysL} is $\mathcal{M}$ of Eq.~\eqref{eq:control} made concrete,
\[
\mathcal{M}(L,S;\alpha) \;=\; \alpha\,\lambda_S(L) \;+\; (1-\alpha)\,\lambda_C(t) \;-\; \kappa L,
\]
i.e.\ the two evidence streams of \S\ref{sec:dial}, weighted by the dial, plus a recalibration leak; the dependence on the raw situation $S(t)$ is carried entirely by the channels ($a_S$, $\xi(t)$, and the perturbations used in the experiments), while the dependence on $L$ itself enters through the ruminative gain $r\,\sig(L)$. Equation \eqref{eq:sysA} is $\mathcal{G}$ made concrete, and its coupling to the belief is real but deliberately routed through the controller: the relaxation $-\gamma(\alpha-\alpha_0)$ carries the person's habitual set-point, while $\mathcal{G}$'s dependence on $(L,\dot L)$ is exactly the feedback term $u(t)$, which reads the belief and its rate of change and pushes the dial back when they signal imbalance (Eq.~\eqref{eq:pivot} below). So the dial \emph{does} adjust in response to what the belief is doing---and the chapter locates that responsiveness in precisely one place, the pivot triggers, which is why the model does too. Figure~\ref{fig:mechanism} draws the whole circuit.

The leak $-\kappa L$ (Table~\ref{tab:notation}) is the standing, low-gain version of the micro-cycle's fifth step, \emph{recalibrate confidence} (Figure~1 of the chapter, p.~5; Box~1, p.~11): the chapter defines calibration as adjusting one's judgment rules so that subjective confidence better aligns with post-hoc evidence and outcomes (footnote~1, p.~3), and a steady pull of the log-odds back toward equipoise, absent fresh evidence, is the simplest continuous implementation of that adjustment. Its size is clinically meaningful: a small $\kappa$ is a large ``confidence gain'' in the chapter's own parameter list (p.~9)---certainty that outruns evidence (Table~\ref{tab:params}).
% [2026-07-07 반영 C6: κ의 의미를 원문 근거(micro-cycle 5단계 "recalibrate confidence", fn.1 p.3의 calibration 정의, p.9 confidence gain)로 설명.]
...
% [2026-07-07 반영 C3: confidence 정의를 §2.2에서 이곳(실제 사용처)으로 이동하고 설명을 보강.]
Two read-outs recur throughout. The \emph{commitment} is the decision of \S2.1 ($\hat h = \arg\max$). The \emph{confidence} is Proust's readable procedural feeling (p.~3) made numerical: the distance of the belief from the undecided point,
\begin{equation}
...
\textbf{Claim tested (p.~6):} pivots ``recenter the trajectory within the adaptive corridor.'' \textbf{Setup:} the over-internalizing patient of Exp.~1, run with the (hysteretic) pivot law \eqref{eq:pivot} on vs.\ off, same noise seed. \textbf{Result} (Fig.~\ref{fig:exp3}): without intervention the belief locks at $\approx0.97$; with the pivot, once $b$ rises past the engage threshold $\theta_H^{\text{on}}$ the dial $\alpha$ is pushed down, disconfirming cues re-enter, and the belief is pulled back toward the corridor, releasing only when $b$ falls below $\theta_H^{\text{off}}$. The intermittent firing of $\alpha$ is the repeated micro-cycle---the model's picture of \emph{why} the loop must be brief and iterated (p.~6)---while the hysteresis band keeps it from chattering.
...
\textbf{Claim tested (p.~7):} tracking \emph{confidence--outcome congruence} and \emph{tolerance of ambiguity} suffices to read out balance. \textbf{Setup:} for each regime I run a truth-balanced ensemble (the hidden state is threat or benign with equal probability) and, at commitment, record confidence and correctness; congruence is summarized by the Expected Calibration Error (ECE, the average gap between confidence and accuracy). Separately, I present a genuinely ambiguous stimulus (internal and external evidence cancel, elevated noise) and measure how long the agent stays with it before confidence crosses a commit level---the model's analogue of the opt-out paradigms used to demonstrate procedural metacognition behaviorally \citep{Proust2012}. \textbf{Result} (Fig.~\ref{fig:exp5}): on calibration, the balanced regime is clearly best (lowest ECE, its reliability curve hugging the diagonal), while both extremes are confidently wrong---the over-internalizer's is the chapter's ``persistent overconfidence $\rightarrow$ suspect internal excess'' (Table~2, p.~11). On tolerance of ambiguity, the ranking does \emph{not} track balance directly: the over-internalizer commits fastest of the three (lowest tolerance)---in this model, jumping to conclusions on thin evidence is what the ruminative gain $r\,\sig(L)$ produces once it dominates the leak---while the over-externalizer stays with the ambiguous stimulus longest (highest tolerance), and the balanced regime falls in between. This is a genuine tension with the chapter's own reading of the index, which pairs low tolerance with ``external avoidance/jumping-to-conclusions tendency'' (Table~2, p.~11)---i.e.\ the opposite axis. Calibration alone carries the balance signal in this realization; \S\ref{sec:disc} and the accompanying model audit flag which axis JTC belongs to as an open question for the collaboration. Even so, the two low-burden indices jointly separate all three regimes.
